package objects
{
	...
	public class GameBackground extends Sprite
	{
		...
		private function onEnterFrame(event:Event):void {
			...
			if (bgLayer1.x < -stage.stageWidth) bgLayer1.x = -5;
			
			bgLayer2.x -= Math.ceil(_speed * bgLayer2.parallax);
			if (bgLayer2.x < -stage.stageWidth) bgLayer2.x = -5;
			
			bgLayer3.x -= Math.ceil(_speed * bgLayer3.parallax);
			if (bgLayer3.x < -stage.stageWidth) bgLayer3.x = -5;
			
			bgLayer4.x -= Math.ceil(_speed * bgLayer4.parallax);
			if (bgLayer4.x < -stage.stageWidth) bgLayer4.x = -5;
